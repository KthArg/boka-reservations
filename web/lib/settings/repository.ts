...
export async function getBusinessSettings(): Promise<BusinessSettings> {
  ...
  const { data, error } = await supabase
    ...
    .select('minimum_decision_window_hours, updated_at')
...
export async function updateDecisionWindow(hours: number, actorId: string): Promise<boolean> {
  const supabase = await createSupabaseServerClient();
  const { data, error } = await supabase
    .from('business_settings')
    .update({ minimum_decision_window_hours: hours, updated_by: actorId })
    .eq('id', BUSINESS_SETTINGS_ID)
    .select('id');
  if (error) {
    console.error('[settings] no se pudo actualizar la ventana de decisión:', error.code, actorId);
    return false;
  }
