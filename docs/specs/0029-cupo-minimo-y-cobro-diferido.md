...
**Actualización 2026-08-13 (respuesta parcial de OnvoPay).** Soporte confirmó que la cancelación de intents existe, pero que **solo aplica sobre `requires_payment_method` y `requires_capture`** — no mencionó `requires_action`, que es exactamente el estado del que depende esta regla. Si se confirma que un intent en 3DS no se puede cancelar, la ventana de recuperación **no puede** crear un intent nuevo sin arriesgar el doble cobro que §5.6 declara irreparable.

**Regla de contingencia**, a adoptar salvo que la verificación en sandbox demuestre que sí se puede cancelar en `requires_action`: mientras exista un intent vivo en ese estado, la reserva **no genera intents nuevos**; el único camino es que el turista complete ese mismo intent por el enlace enviado. Vencida la ventana sin autenticación, la reserva se **cancela** en lugar de reintentarse. Preserva el invariante anti-doble-cobro por construcción, al costo de no poder recuperar con otra tarjeta una reserva abandonada en 3DS.
