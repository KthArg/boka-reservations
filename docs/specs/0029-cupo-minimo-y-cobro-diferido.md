...
- **Prueba con tarjeta real antes de abrir a turistas — deuda de pre-lanzamiento** (decidida por el usuario, 2026-09-15). El sandbox no interactúa con redes bancarias reales, así que no puede mostrar cómo responde un banco a un cobro con tarjeta guardada sin el cliente presente. Con todo listo para producción, y antes de activar `DEFERRED_CHARGE_ENABLED` para turistas, se hace una prueba en modo live con una tarjeta virtual real del usuario, creada solo para eso: cobros de $1 a los días 0, 7 y 30, cada uno reembolsado. **Dura 30 días: tiene que empezar al menos un mes antes del lanzamiento.** Procedimiento, reglas de seguridad y criterios de decisión en `docs/cutover-produccion.md`, Fase 6b.
