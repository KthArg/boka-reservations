...
- **Unit (enmascarado)**: `maskEmail` cubre casos normales (`juan@gmail.com` → `j***@gmail.com`), local de 1 carácter, entrada sin `@`, y cadena vacía → devuelve cadena vacía.
- **Integración/render (página de éxito)**: el HTML renderizado por la página de éxito para una reserva dada NO contiene el `customer_email` completo ni el `customer_name` de esa reserva; sí contiene el email enmascarado, el código corto y el nombre del tour.
- **Test manual documentado en el PR**: recorrer el checkout, intentar enviar sin marcar el checkbox (bloqueado en cliente), completar el pago con tarjeta de prueba, y verificar en la página de éxito que no aparecen nombre ni email completo (solo el enmascarado).
