...
| `users`                    | authenticated SELECT (`auth/server.ts`, `users/repository.ts`, `guides/repository.ts`); escrituras por service | `GRANT SELECT TO authenticated` (anon ya revocado en `…009`; mantenerlo así)                                             |
...
`SECURITY INVOKER`, solo lectura, con `REVOKE EXECUTE … FROM anon, authenticated`. El test
...
- **Función nueva**: `public.audit_table_grants_to_public_roles()` (SECURITY INVOKER, solo lectura,
