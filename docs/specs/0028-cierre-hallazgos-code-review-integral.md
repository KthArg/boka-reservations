...
- **Mapping exacto estado → outcome** (después del gate por evento, con la reserva bajo `FOR UPDATE`):
  ...
  - `cancelled` → **camino nuevo `late_payment_refunded`**: marca el pago `succeeded` (espejo del camino overbooked: el refund necesita un pago succeeded que reembolsar), encola refund total en la misma transacción (INSERT en `refunds` respetando el índice único parcial de refund activo por reserva; si ya existe uno activo, no duplica) + audit `booking.late_payment_refunded`. La reserva NO cambia de estado aquí; cuando el refund se acredite, `settle_refund` la lleva a `refunded` (semántica existente de 0011 — ver §7).
...
- **`markProcessing` falla tras POST exitoso**: Sentry crítico con `external_refund_id`; el guard stale lo lleva a `failed('processing-stale')`; el retry manual entra por el camino verify (tiene id) → no re-crea.
