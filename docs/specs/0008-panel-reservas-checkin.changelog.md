...
- Gotchas resueltos al ejecutar la integración: (1) el assert de RLS anon asumía la tabla vacía y fallaba en la suite completa — se reescribió para verificar que anon no ve la reserva concreta recién creada (RLS de bookings correcta: anon = 0 filas, confirmado vía psql `SET ROLE`, REST y supabase-js). (2) el assert de idempotencia comparaba strings y PostgREST formatea `+00:00` (no `.000Z`) — se compara por instante (`getTime()`).
